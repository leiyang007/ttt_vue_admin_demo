<template>
	<div class="edit-role">
		<div class="nav-list">
			...
			/<span class="gray-font bh-title">Edit Commodity</span>
		</div>
		<div class="edit-cont">
...
export default{
	...
	methods:{
        ...
        saveRole(){
			...
			console.log(parm)		
			this.$axios.put('/v1/category', parm)
			  .then((res) => {			  	
			  	let nowData = res.data
       ...
       getCommodityData(){
       		let _this = this			
			let parm = this.$route.query.commodityId
			
			console.log(JSON.stringify(parm))
			
			this.$axios.get('/v1/category/'+parm)
				
			  .then((res) => {	  	
			  	let nowData = res.data
			  	
			  	if(nowData.code == 1){
			  		_this.ruleForm.departmentId = nowData.data.departmentId
			  		_this.ruleForm.commodity = nowData.data.name
