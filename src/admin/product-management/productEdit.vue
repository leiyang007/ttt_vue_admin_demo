<template>
	<div class="edit-role">
		<div class="nav-list">
			...
			/<span class="gray-font bh-title">Edit Product</span>
		</div>
		<div class="edit-cont">
...
export default{
	...
	methods:{
        ...
        save(){
			...
			console.log(parm)		
			this.$axios.put('/v1/product', parm)
			  .then((res) => {			  	
			  	let nowData = res.data
       ...
       getProductData(){
       		let _this = this
       		let parm = this.$route.query.productId
			this.$axios.get('/v1/product/'+parm)
				
			  .then((res) => {			  	
			  	let nowData = res.data
			  	if(nowData.code == 1){
			  		let dataObj = nowData.data
			  		let clusterArr = dataObj.clusterList
			  		let arr = []
			  		
			  		for(let i=0; i<clusterArr.length; i++){
			  			if(clusterArr[i].has == 1){
			  				arr.push(clusterArr[i].id)
			  			}
			  		}
			  		
				  	_this.ruleForm.departmentId = dataObj.departmentId
				  	_this.ruleForm.cluster = arr
				  	_this.ruleForm.commodity = dataObj.categoryId.toString()
				  	_this.ruleForm.item = dataObj.productName
				  	_this.ruleForm.upc = dataObj.code
				  	_this.description = dataObj.productDesc
				  	_this.clusterList = clusterArr
				  	
				  	_this.getCommodity(_this.ruleForm.departmentId)
			  	}else{
			  		console.log(nowData.errorInfo)
			  		this.$message({
			          message: nowData.errorInfo,
