	...
	<div class="report">
		...
		<div>
			<div class="nav-head-one">
				<ul class="clear">
					<li class="red-font">Average Item Movement Report</li>
				</ul>
			</div>
		</div>
		...
		<div class="table-body" style="word-wrap:break-word;">	
			<template>				
			  <el-table ref="multipleTable" v-loading="loading" :data="tableData" tooltip-effect="dark" border :header-cell-style="tableHeaderColor">			    
			    ...
			    <el-table-column prop="weeklyAverage" label="Weekly Average" min-width="200" :formatter="weeklyAverage">
			    </el-table-column>		    
			  </el-table>
			    <el-pagination
...
export default{
	...
	methods:{	
		...
		weeklyAverage(rows){
			//return rows.weeklyAverage.toFixed(0)
			return Math.round(rows.weeklyAverage)   //四舍五入取整
		},
		search(){
			this.nowPage = 1
			this.getTableData()
		...
		getTableData(){
			...
			this.$axios.get('/v1/report/averageItemMovementReport',{params: parm})
