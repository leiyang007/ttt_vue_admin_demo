<template>
	<div class="edit-role">
		<div class="nav-list">
			...
			/<span class="gray-font bh-title">Edit Role</span>
		</div>
		<div class="edit-cont" v-loading="loading">
...
export default{
	...
	methods:{
		...
		getRoleTree(){
			let _this = this			
			let parm = this.$route.query.roleId
			
			console.log(JSON.stringify(parm))
			
			this.$axios.get('/v1/role/'+parm)
				
			  .then((res) => {	  	
			  	let nowData = res.data
			  	
			  	if(nowData.code == 1){
			  		_this.ruleForm.roleName = nowData.data.roleName
			  		_this.description = nowData.data.description
					_this.dataTree = nowData.data.moudleTrees
					_this.loading = false
			  	}else{
